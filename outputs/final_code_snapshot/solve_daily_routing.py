...
- daily_route_failures.csv, only if failures occur and --allow-skipped-streams is used
"""
...
def stop_map_from_nodes(nodes_df: pd.DataFrame) -> dict[str, int]:
    ...
    for row in nodes_df.itertuples(index=False):
        ...
        node_type = str(getattr(row, "node_type", "")).strip().lower()

        if node_type == "stop" and stop_id is not None:
...
def solve_ortools_vrp(
    ...
    depot_index: int = 0,
    routing_time_limit_sec: int = 300,
) -> dict[str, Any] | None:
    ...
    def time_callback(from_index: int, to_index: int) -> int:
        ...
        travel_time = int(matrix_minutes[from_node][to_node])
        service_time = int(service_minutes[from_node])
    ...
    def gal_callback(from_index: int) -> int:
        return int(gallon_demands[manager.IndexToNode(from_index)])
    ...
    def lb_callback(from_index: int) -> int:
        return int(pound_demands[manager.IndexToNode(from_index)])
    ...
    for vehicle_id, cap in enumerate(vehicle_time_caps):
        routing.solver().Add(time_dim.CumulVar(routing.End(vehicle_id)) <= int(cap))

    search = pywrapcp.DefaultRoutingSearchParameters()

    # Better for capacity-constrained VRP than PATH_CHEAPEST_ARC.
...
def solve_with_stop_dropping(
    ...
    # If a stream-day has fewer stops than min_stops_to_keep,
    # still allow routing instead of immediately failing.
    effective_min_stops_to_keep = min(min_stops_to_keep, len(working))

    while len(working) >=  effective_min_stops_to_keep:
        ...
        for old_i in kept_local_nodes:
            ...
            for old_j in kept_local_nodes:
                row.append(int(matrix_minutes_full[old_i][old_j]))
...
def main() -> None:
    ...
    for (day, stream), grp in grouped:
        ...
        if result is None:
            ...
            print(f"scheduled_bins = {len(grp)}")
            print(f"scheduled_stops = {len(stop_rows)}")
            print(f"active_trucks = {num_trucks}")
            print(f"trucks = {trucks}")
            print(f"total_pickup_gal = {total_gal}")
            print(f"total_pickup_lb = {total_lb}")
            print(f"total_service_min_only = {total_service_min}")
            print(f"base_volume_cap = {base_vol_cap}")
            print(f"total_effective_gal_cap = {total_effective_gal_cap}")
            print(f"total_effective_lb_cap = {total_effective_lb_cap}")
            print(f"vehicle_gal_caps = {vehicle_gal_caps}")
            ...
            if not stop_agg.empty:
                ...
                print(
                    stop_agg.sort_values(
                        ["stop_pickup_gal", "stop_pickup_lb"],
        ...
        for vehicle_idx, route in enumerate(result["routes"]):
            ...
            for reduced_node, original_node in zip(route, original_route):
                ...
                serials_here = (
                    stop_bins_map.get(stop_id, None) if stop_id is not None else None
                )
